...
<template>
  <div class="h-full flex flex-col text-xs overflow-hidden" :style="{
    ...
    <div class="text-lg tracking-wider leading-tight">{{ section.card_id }}</div>
    ...
    <div class="h-2/4 flex flex-row items-center justify-between px-2 font-bold uppercase">
      ...
      <div class="text-center">
        <div class="text-lg tracking-wider leading-tight">{{ cardName }}</div>
      </div>
    ...
    <div class="h-4/8 grid grid-rows-3 gap-1 p-2 items-center">
      <div v-for="(action) in section.actions" :key="action.id"
        class="flex items-center gap-3 p-2 bg-white/20 rounded-sm w-full h-full max-h-[40px]">
        {{ action.id }}
